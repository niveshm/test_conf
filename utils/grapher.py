...
class Grapher(object):
    def __init__(self, dataset, data_type = "all", sample_perc=1.0):
        """
        ...
        if data_type == "all" or data_type == "test":
            self.train_idx = self.create_store("train.txt", sample_perc)
            self.valid_idx = self.create_store("valid.txt", sample_perc)
            self.test_idx = self.create_store("test.txt", sample_perc)
    ...
    def create_store(self, file, sample_perc):
        """
        ...
        if sample_perc < 1.0:
            num_sample = int(len(store_idx) * sample_perc)
            ## sample with prob without replacement, give more prob to latest edges
            # sample_indices = np.random.choice(len(store_idx), num_sample, replace=False)
            prob = np.exp(store_idx[:,3]) / np.sum(np.exp(store_idx[:,3]))
            sample_indices = np.random.choice(len(store_idx), num_sample, replace=False, p=prob)
            store_idx = store_idx[sample_indices]
