...
dataset = "icews14"
data_type = "valid"
path_file = f"./preds/{dataset}/{data_type}_edge_paths_notime.pkl"
...
def create_query(query_edge, paths, y):
    ...
    for path in paths:
        ...
        for i in range(1, len(path), 2):
            ...
            context_text += f"entity '{prev_ent}' is connected to entity '{ent}' by the relation '{rel}' at time sometime t{cnt}. "
            prev_ent = ent
            cnt += 1
        
        context_text += f"Note that: t1 <= t2 <= ... <= tn < {ts}. "
...
with open(f"./preds/{dataset}/{data_type}_llm_data_notime.txt", "w") as f:
    f.write(query_data)
